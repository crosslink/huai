...
class CKunenaShowcat {
	...
	function __construct($catid, $page=0) {
		 ...
		 */
		/*******************  BEGIN *************************/
		$query_for_subcategories = "select id from #__kunena_categories WHERE ( parent={$this->db->Quote($this->catid)} and published='1') or 
									parent in (select id from #__kunena_categories where parent={$this->db->Quote($this->catid)} and published='1')
									";
		...
		$this->db->setQuery ( "SELECT COUNT(*) FROM #__kunena_messages WHERE parent='0' AND catid IN ({$subcatidsstr}) AND hold IN ({$hold})" );
	...
	WHERE t.parent='0' AND t.hold IN ({$hold}) AND t.catid IN ({$subcatidsstr}) AND m.hold IN ({$hold}) AND m.catid IN ({$subcatidsstr})
	...
	*/
	...
	function displaySubCategories1() {
		require_once (KUNENA_PATH_FUNCS . '/listcat.php');
		$obj = new CKunenaListCat($this->catid);
		$obj->loadCategories();
		if (!empty($obj->categories [$this->catid])) {
			$obj->displayCategories1();
			$this->hasSubCats = '1';
		}
	}
